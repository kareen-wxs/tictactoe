...
- **Важно:** Перед игрой напишите боту команду `/start` в Telegram, чтобы получать промокоды!
- Вы играете за X (розовый цвет)
- Компьютер играет за O (фиолетовый цвет)
- При победе вы получаете 5-значный промокод на скидку (на экране и в Telegram)
- При проигрыше появляется предложение сыграть ещё раз

### Инструкция для игроков

Перед началом игры каждый игрок должен:
1. Найти бота в Telegram
2. Написать боту команду `/start`
3. Только после этого начинать играть

Без команды `/start` промокоды не будут приходить в Telegram (но будут показываться на экране).

Подробная инструкция в файле `INSTRUCTIONS.md`
